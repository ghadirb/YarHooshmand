...
object BackupUtils {
    suspend fun exportBackup(context: Context): List<ReminderEntity> =
        withContext(Dispatchers.IO) { AppDatabase.getInstance(context).reminderDao().getAllOnce() }

    suspend fun importBackup(context: Context, reminders: List<ReminderEntity>) =
        withContext(Dispatchers.IO) { AppDatabase.getInstance(context).reminderDao().insertAll(reminders) }

    suspend fun tryUploadBackupSilent(context: Context, reminders: List<ReminderEntity>): Boolean = true
}
